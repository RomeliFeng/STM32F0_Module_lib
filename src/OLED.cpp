...
void OLED::Init() {
	...
	GPIOInit();
	SPI.init();
	GPIO_WriteBit(OLED_GPIOx(OLED_CS_PORT_NUMBER),
			OLED_PIN_MASK(OLED_CS_PIN_NUMBER), Bit_RESET);
	...
	Write(0xa1, Cmd_Mode); //--Set SEG/Column Mapping     0xa0���ҷ��� 0xa1����
	Write(0xc8, Cmd_Mode); //Set COM/Row Scan Direction   0xc0���·��� 0xc8����
...
void OLED::print(uint8_t x, uint8_t y, uint8_t* str,
	...
	while (x <= (128 - (128 % mode) - mode) && *str != '\0') { //超过长度自动舍弃
...
void OLED::print(uint8_t x, uint8_t y, long num, CharMode_Typedef mode) {
	uint8_t str[20];
	byNumber(num, 10, str);
	print(x, y, str, mode);
}

void OLED::print(uint8_t x, uint8_t y, float f, uint8_t ndigit,
		CharMode_Typedef mode) {
	uint8_t str[20];
	byFloat(f, ndigit, str);
	print(x, y, str, mode);
}
...
void OLED::print_c(uint8_t x, uint8_t y, uint8_t c,
		CharMode_Typedef mode) {
	SetPos(x, y);
	switch (mode) {
	case C6x8:
...
void OLED::GPIOInit() {
	...
	RCC_AHBPeriphClockCmd(OLED_RCC_MASKx(OLED_DC_PORT_NUMBER), ENABLE);
	RCC_AHBPeriphClockCmd(OLED_RCC_MASKx(OLED_RESET_PORT_NUMBER), ENABLE);
	RCC_AHBPeriphClockCmd(OLED_RCC_MASKx(OLED_CS_PORT_NUMBER), ENABLE);
	...
	GPIO_InitStructure.GPIO_Pin = OLED_PIN_MASK(OLED_CS_PIN_NUMBER);

	GPIO_Init(OLED_GPIOx(OLED_CS_PORT_NUMBER), &GPIO_InitStructure);
